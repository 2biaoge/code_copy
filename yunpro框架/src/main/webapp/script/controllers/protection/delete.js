...
controllers.controller('protecDeleteCtrl',
    ["firewallServices", 'items', 'ErrorServices',
        '$scope', '$modalInstance','$route','$rootScope','$timeout',
        function(firewallServices, items,ErrorServices,
                 $scope, $modalInstance,$route,$rootScope,$timeout) {
            $scope.modalConfirm = {
                title: '删除云防护',
                textLine1: '您确定要删除云防护吗？'
            };
            ...
            $scope.ok = function () {
                ...
                for(var i = 0; i < items.length; i++){
                    firewallServices.deleteitem(items[i])
                        .then(
                        function(data){
                            if(data.result === 'true'){

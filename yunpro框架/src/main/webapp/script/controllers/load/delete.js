...
/*删除*/
controllers.controller('loadDeleteCtrl',
    ['loadServices', 'items', 'ErrorServices',
        '$scope', '$modalInstance','$route','$rootScope','$timeout',
        function(loadServices, items, ErrorServices,
                 $scope, $modalInstance,$route,$rootScope,$timeout) {

            $scope.modalConfirm = {
                title: '删除',
                textLine1: '你确定要删除负载均衡吗？'
            };
            $scope.ok = function () {
                $modalInstance.close();
                ...
                for(var i = 0; i < items.length; i++){
                    loadServices.deleteitem(items[i]).then(
                        function(data){
                            if(data.result === 'true'){
